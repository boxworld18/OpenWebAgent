...
class HtmlParser():
    ...
    def parse(self, root: html.HtmlElement, keep: list[str], obs: list[str], parent_chain: bool=False, get_new_label: bool=False) -> dict[str]:
        ...
        def _dfs(node: html.HtmlElement, keep: list[str]=[], obs: list[str]=[], 
            ...
            if label:
                in_keep_list = True
                # print(label, visible, in_keep_list, in_obs_list, keep_element, self.rect.get(bid, None))
            ...
            # don't keep our dom marker
            node_class = node.get("class", "")
            for cls in ['our-dom-marker', 'our-dom-marker-text']:
    ...
    def parse_tree(self) -> dict[str]:
        ...
        all_node = sum(1 for _ in root.iter('*'))
        ...
        kept = len(cmsg.get('label_element', []))
        print(f"[Node] {all_node}")
        print(f"[Keep] {kept}")
        print(f"[E.redn] {100 - (kept) * 100.0 / all_node}")
