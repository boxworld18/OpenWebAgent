...
const HintCoordinator = {
    ...
    removeHintMarkers(){
        this.linkHintsMode.removeHintMarkers();
    },
...
class LinkHintsMode {
    ...
    createMarkerFor(desc) {
        ...
        if (desc.frameId === frameId) {
            ...
            el.setAttribute('vimium-label-z-index', el.style.zIndex);
            localHintDescriptor.element.setAttribute('vimium-z-index', el.style.zIndex);
            localHintDescriptor.element.classList.add("clickable-element",);
    ...
    getLocalHints(requireHref) {
        ...
        while ((visibleElement = visibleElements.pop())) {
            ...
            if (
                ...
                (element.contains(elementFromMiddlePoint) || elementFromMiddlePoint.contains(element)) || true
            ) {
                // 把此处条件加上|| true
...
function AutoHint() {
        // 页面加载完成后，延迟两秒进行标记
    if(!document.getElementById("vimium-marker")){
        var marker = document.createElement("button")
        marker.setAttribute("id", "vimium-marker")
        marker.innerText = "click";
        marker.style.display = 'none';
        document.body.append(marker);
        document.getElementById("vimium-marker").addEventListener("click", function () {
            LinkHints.activateMode(count = 99999, {
                mode: HintCoordinator.linkHintsMode,
                registryEntry: {
        ...
        var removeMarker = document.createElement("button")
        removeMarker.setAttribute("id", "vimium-remove-marker")
        removeMarker.innerText = "click";
        removeMarker.style.display = 'none';
        document.body.append(removeMarker);
        document.getElementById("vimium-remove-marker").addEventListener("click", function () {
...
DomUtils.documentReady(AutoHint)
